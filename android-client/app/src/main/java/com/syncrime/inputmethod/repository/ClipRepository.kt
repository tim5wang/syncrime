...
class ClipRepository(
     ...
     * 获取所有剪藏
     */
    fun getAllClips(): Flow<List<KnowledgeClip>> {
        return clipDao.getAll(20, 0)
    }
    ...
    suspend fun getStats(): ClipStats {
        // DAO 返回 Flow，需要获取第一个值
        val total = clipDao.getTotalCount().first()
        val today = clipDao.getTodayCount().first()
